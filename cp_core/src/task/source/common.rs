...
use super::config::{CsvSourceConfig, HttpSourceConfig, JsonSourceConfig, MySqlSourceConfig, PostgresSourceConfig, SourceGroupConfig};
...
impl SourceGroupConfig {
    fn parse_subsources(&self) -> Vec<Result<Box<dyn SourceConfig>, CpError>> {
        self.sources
            .iter()
            .map(|transform| {
                let config = try_deserialize_stage!(transform, dyn SourceConfig, JsonSourceConfig, CsvSourceConfig, HttpSourceConfig, MySqlSourceConfig, PostgresSourceConfig);
                config.ok_or_else(|| {
                    CpError::ConfigError(

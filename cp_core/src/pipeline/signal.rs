...
use crate::{frame::common::{FrameUpdateInfo, FrameUpdateType}, util::error::{CpError, CpResult}};
...
impl SignalState {
    pub fn new() -> Self {
        ...
        Self { sig_sender , sig_recver, sigterm_stream, state_type: SignalStateType::Alive }
    }

    pub async fn send_replace_signal(&self) -> CpResult<()> {
        match self.sig_sender.send(FrameUpdateInfo { source: "REPLACE".to_owned(), timestamp: Utc::now(), msg_type: FrameUpdateType::Replace }).await {
            Ok(_) => Ok(()),
    ...
    pub async fn send_termination(&self) -> CpResult<()> {
        match self.sig_sender.send(FrameUpdateInfo { source: "SIGTERM".to_owned(), timestamp: Utc::now(), msg_type: FrameUpdateType::Kill }).await {
            Ok(_) => Ok(()),
        ...
        loop {
            ...
            match curr_state {
                SignalStateType::Alive => {
                    log::warn!("Stages will terminate after completing their current event cycle. Ctrl-C again to force-kill");
                    self.state_type = SignalStateType::RequestedKill;
                    self.send_termination().await.expect("Failed to send termination signal to stages");
                }

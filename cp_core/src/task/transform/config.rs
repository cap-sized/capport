...
use crate::{parser::dtype::DType, task::config::{PolarsExprKeyword, StrKeyword}};

#[derive(Debug, Clone, PartialEq, Eq, Deserialize)]
pub struct FormatActionConfig {
    pub template: StrKeyword,
    pub columns: Vec<StrKeyword>,
}

#[derive(Debug, Clone, PartialEq, Eq, Deserialize)]
pub struct ConcatActionConfig {
...
#[derive(Debug, Clone, PartialEq, Eq, Deserialize)]
pub struct LiteralActionConfig {
    pub dtype: DType,
...
pub struct _JoinTransformConfig {
    ...
    pub right: StrKeyword, // this is the right frame to join on
...
mod tests {
    ...
    use crate::task::{config::{Keyword, PolarsExprKeyword, StrKeyword}, transform::config::_JoinTransformConfig};
...
";
        ...
        assert_eq!(actual, SelectTransformConfig {
            select: HashMap::from([
                (StrKeyword::with_value("key".to_owned()), PolarsExprKeyword::with_value(col("value"))),
                (StrKeyword::with_value("key2".to_owned()), PolarsExprKeyword::with_symbol("symbol")),
                (StrKeyword::with_symbol("symbol"), PolarsExprKeyword::with_value(col("value"))),
                (StrKeyword::with_value("nested_key".to_owned()), PolarsExprKeyword::with_value(col("nested").struct_().field_by_name("value"))),
                (StrKeyword::with_value("123".to_owned()), PolarsExprKeyword::with_value(col("isvalid"))),
                (StrKeyword::with_value("nest.k".to_owned()), PolarsExprKeyword::with_value(col("left_is_str"))),
            ])
        });
    }
...
";
        ...
        assert_eq!(actual, JoinTransformConfig {
...
";
        ...
        assert_eq!(actual, JoinTransformConfig {
            join: _JoinTransformConfig {
                ...
                right_select: Some(HashMap::from([
                    (StrKeyword::with_value("test".to_owned()), PolarsExprKeyword::with_symbol("one")),
                    (StrKeyword::with_symbol("another"), PolarsExprKeyword::with_value(col("two"))),
                ])),
            }
        });
    }
    ...
    fn parse_transform_drop() {
        ...
        assert_eq!(actual, DropTransformConfig {
            drop: vec![

...
pub fn pack_configurables(
    ...
    let top_level_map = serde_yaml_ng::from_value::<HashMap<String, serde_yaml_ng::Value>>(yaml_root)?;
    for (configurable_name, value) in top_level_map {
        let node_fields = serde_yaml_ng::from_value::<HashMap<String, serde_yaml_ng::Value>>(value)?;
